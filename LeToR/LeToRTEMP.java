...
public class LeToRTEMP {
    ...
    public static void main(String[] args) throws Exception {
        ...
        // Evaluate Query
        Map<String, Double> pagerankMap = readPageRank();
        ...
        if (true) {
            ...
            while (scanner.hasNext()) {
                ...
                // Get features for its training docs.
                List<List<Double>> partialFeatures = readTrainingDocs(queryNum, pagerankMap);
                features.addAll(partialFeatures);
    ...
    private static void normalizeFeature(List<List<Double>> features) {
        double min = Double.MAX_VALUE;
        double max = Double.MIN_VALUE;

        for (List<Double> list : features) {
            for (Double val : list) {
                if (val > max) {
                    max = val;
                }
        ...
        for (int i = 0; i < features.size(); i++) {
            for (int j = 0; j < features.get(i).size(); j++) {
                features.get(i).set(j, (features.get(i).get(j) - min) / (max - min));
            }
        }
    }
...
//        }
    ...
    private static Map<String, Double> readPageRank() throws FileNotFoundException {
        Scanner sc = new Scanner(new FileReader("pagerank.txt"));
        Map<String, Double> hashMap = new HashMap<String, Double>();
    ...
    /**
     * Get the features for this query given the page  rank.
     */
    private static List<List<Double>> readTrainingDocs(int  queryNum, Map<String, Double> hashMap) throws Exception {
        List<List<Double>> res = new ArrayList<List<Double>>();
        boolean hasDocs = false;

        Scanner sc = new Scanner(new FileReader("train.score"));
        ...
        while (sc.hasNext()) {
            ...
            if (qid == queryNum) {
                ...
                // Add tf
                for (int val : tv.stemsFreq) {
                    features.add((double) val);
                }

                // Add page rank
                features.add(hashMap.get(externalDocid));

                // Add label
                features.add((double) relevance);
    ...
    private static void writeFeatures(List<List<Double>> res) throws FileNotFoundException {
        PrintWriter pw = new PrintWriter(new File("features.txt"));
