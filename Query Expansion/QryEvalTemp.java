...
public class QryEvalTemp {
	...
	public static void main(String[] args) throws Exception {
		...
		// Imput: 1(numParam) params result
...
//		}
	...
	static String getExternalDocid(int iid) throws IOException {
		Document d = QryEvalTemp.READER.document(iid);
	...
	static int getInternalDocid(String externalId) throws Exception {
		...
		IndexSearcher searcher = new IndexSearcher(QryEvalTemp.READER);
	...
	static Qryop parseQuery(String qString) throws IOException {
		...
		while (tokens.hasMoreTokens()) {
			...
			if (token.matches("[ ,(\t\n\r]")) {
				...
				int dist = Integer.valueOf(matcher.group(1));
	...
	static void writeResults(String queryName, QryResult result, String path) throws IOException {
		...
		if (result.docScores.scores.size() < 1) {
			...
			for (int i = 0; i < result.docScores.scores.size(); i++) {
				write.write("1 Q0 " + getExternalDocid(result.docScores.getDocid(i)) + " " + i + " " + result.docScores
						.getDocidScore(i) + " run-1\n");
			}
		}
	}
}
